...
# ── Sleep prevention ──────────────────────────────────────────────────────────
def prevent_sleep():
    """Prevent the OS sleeping during the pipeline run."""
    if OS == 'Windows':
        ...
        print('ℹ️  Linux: launch with systemd-inhibit to prevent sleep')


def allow_sleep():
    """Re-enable OS sleep after pipeline completes."""
    if OS == 'Windows':
        import ctypes
...
def get_device() -> str:
    if torch.cuda.is_available():
        ...
        print(f'✅ CUDA GPU detected: {name} ({vram:.1f} GB VRAM)')
...
works = pd.read_csv(f'{DATA_PATH}/goodreads_works.csv')
...
reviews = pd.read_csv(f'{DATA_PATH}/goodreads_reviews.csv', low_memory=False)
